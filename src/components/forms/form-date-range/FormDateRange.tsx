            ...
            <div className="relative">
              <Popover>
                <Popover.Trigger id={name} asChild>
                  <Button
                    className={cn(
                      'aria-expanded:bg-accent/50 block',
                      error &&
                    ...
                    <div className="flex grow items-center gap-2">
                      ...
                      <Conditional>
                        <Conditional.If condition={!!date}>
                          <Conditional.If condition={!!date?.from && !date?.to}>
                            {formatDate(date?.from, constants.DateFormats.FULL_MONTH_DAY_YEAR)}
                          </Conditional.If>

                          <Conditional.If condition={!!date?.to && !date?.from}>
                            {formatDate(date?.to, constants.DateFormats.FULL_MONTH_DAY_YEAR)}
                          </Conditional.If>
